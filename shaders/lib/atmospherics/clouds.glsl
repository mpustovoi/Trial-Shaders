...
float CloudSampleBase(vec2 coord, vec2 wind, float cloudGradient, float sunCoverage, float dither, float multiplier) {
    ...
    vec2 dynamicWind = wind + vec2(time * windSpeed, sin(time * 0.1) * turbulenceStrength);

	#if CLOUD_BASE == 0
	float noiseBase = texture2D(noisetex, coord * 0.25 + dynamicWind).r;
	#else
	float noiseBase = texture2D(noisetex, coord * 0.5 + dynamicWind).g;
	noiseBase = pow(1.0 - noiseBase, 2.0) * 0.5 + 0.25;
	#endif

	float detailZ = floor(cloudGradient * float(CLOUD_THICKNESS)) * 0.04;
	float noiseDetailLow = texture2D(noisetex, coord.xy * 0.5 - dynamicWind * 2.0 + detailZ).b;
	float noiseDetailHigh = texture2D(noisetex, coord.xy * 0.5 - dynamicWind * 2.0 + detailZ + 0.04).b;
